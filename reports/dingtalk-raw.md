# dingtalk 字段验证报告（Raw）

**生成时间：** 2026-04-09 01:18:20

## 接口：N/A

**样本记录数：** 289

| 字段名 | 类型 | 示例值 | 可空 |
|--------|------|--------|------|
| *红人ID | string | SpiritoftheOutdoors | 否 |
| *跟进人 | string | Windy-吴婷 | 否 |
| *主页链接 | string | https://www.youtube.com/c/SpiritoftheOutdoors | 否 |
| *Email | string | justinpeden@gmail.com | 否 |
| 其他联系方式 | string |  (985) 228-1029  | 否 |
| *合作价格及交付项（格式：合作周期-价格-交付内容） | string | 猎猪渠道，单次合作，$5000插入90s-120s  不同意佣金合作 | 否 |
| *佣金率 | string | {"sequence":"#REF!","identifier":"#REF!","data":"[{\"fxError\":\"#REF!\"}]","filterFactor":["#REF!"]} | 否 |
| *粉丝数（k） | string | 43 | 否 |
| 备注 | string | 红人有自己的销售网站，在网站上挂链接https://frostyfreedom.com/ | 否 |
| *寄样地址 | string | 10251 road 852
Philadelphia MS 39350

Phone 601-416-4224 | 否 |
| *全名 | string | Randy Kunath | 否 |
| 红人等级 | string | B | 否 |
| *合作模式 | string | 单次合作 | 否 |
| *收款信息 | string | $5000/video | 否 |
| *Code | string | SOT18 | 否 |
| *付费模式 | string | 纯样品置换 | 否 |
| 红人类型 | string | 工具, 枪支 | 否 |
| 原UTM | string | https://bit.ly/4sbdhHw | 否 |
| 合作review | string | 价格及交付项不合理。播客价值难以衡量，YouTube账号数据不佳。1月到期后重新谈 | 否 |
| 2025Q4 GMV | string | 90.2 | 否 |
| 后续动作 | string | 重谈价格和交付项 | 否 |
| 2025Q4播放 | string | 5655 | 否 |
| 均播（k） | string | 4.4 | 否 |
| *所在州 | string | Mississippi | 否 |

## 需求字段（待人工对照）

| 需求字段（中文） | 报表 | 对照结果 |
|----------------|------|---------|
| 营销日期 | marketing_performance_table | ⬜ 待确认 |
| 营销投入 | marketing_performance_table | ⬜ 待确认 |
| 渠道名称 | marketing_performance_table | ⬜ 待确认 |
| 转化数 | marketing_performance_table | ⬜ 待确认 |
