...
- ARCH8: `config/field_requirements.yaml` 按报表分组（profit_table / marketing_table 等），每条记录含 `display_name`、`source`、`table` 三字段
...
**Given** 读取 `README.md`
**When** 查看安装步骤
**Then** 包含：环境要求（Python 3.x）、`pip install -r requirements.txt` 命令、`playwright install chromium` 的显式说明（注明该步骤无法通过 requirements.txt 自动完成）、各数据源运行命令示例
...
**Given** `config/field_requirements.yaml` 包含 `profit_table` 和 `marketing_table` 两个分组
